        ...
        function addVtHeader(div, data, cellHeight) {
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                for (var j = 0; j < data[i].series.length; j++) {
					if(data[i].series[j].name == "Progress" && data[i].series[j].dikerjakan == data[i].series[j].volume ){
        ...
        function addBlocks(div, data, cellWidth, start) {
            ...
            for (var i = 0; i < data.length; i++) {
                for (var j = 0; j < data[i].series.length; j++) {
                    ...
                    var size = DateUtils.daysBetween(series.start, series.end) + 1 /30;
					var offset = DateUtils.daysBetween(start, series.start);
					var block = jQuery("<div>", {
                        "class": "ganttview-block",
                        "title": series.name + ", " + parseFloat((data[i].series[j].dikerjakan / data[i].series[j].volume) * data[i].series[j].persen).toFixed(2) + "%",
                        "css": {
					...
					if(data[i].series[j].name == "Progress" && data[i].series[j].dikerjakan == data[i].series[j].volume ){
						block.css("background-color", 'lightgreen');
					}

					if(data[i].series[j].dikerjakan == data[i].series[j].volume){
						 block.append(jQuery("<div>", { "class": "ganttview-block-text" }).text(parseFloat(data[i].series[j].persen).toFixed(2) + "% "));
					
						}else{
						 block.append(jQuery("<div>", { "class": "ganttview-block-text" }).text(parseFloat((data[i].series[j].dikerjakan / data[i].series[j].volume) * data[i].series[j].persen).toFixed(2) + "%"));
							  block.css("background-color", data[i].series[j].color);
						}
